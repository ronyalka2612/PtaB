...
[CustomEditor(typeof(LibStatesController))]
public class LibStatesControllerEditor : Editor
{

    private LibStatesController myTarget;
    ...
    void OnEnable()
    {
        Debug.Log("StatesController OnEnable");
        myTarget = (LibStatesController)target;
    ...
    public override void OnInspectorGUI()
    {
        ...
        /* Note
         * dikurangain -1 pada legnthEnum karena nilai dibawah 0(kosong) ada satu
         * dan  j != legnthEnum-1 digunakan untuk pengecekan jika memilih "Count" pada state
         * 
         */
        int legnthEnum = Enum.GetNames(typeof(LibEdStateUtilities.GameStates)).Length-1;
        for (int j = 0; j < legnthEnum; j++)
        {
            if ((int)myTarget.State == j && j != legnthEnum-1)
            {
        ...
        if (Prop.NextVisible(true))
        {
            ...
            do
            {
                ...
                else if (LibUtilitiesEditor.CurString == "Tab" + (tab - 1) && LibUtilitiesEditor.IsConditional() && sizeByType == 0)
                {
                    ...
                    try
                    {
                        property = soTarget.FindProperty(Prop.name).Copy();
                    }
                    catch
                    {
                        Debug.Log("cek editor : Prop.name:" + Prop.name);
                        Debug.Log("cek editor : property :" + property);
                        property = soTarget.FindProperty(Prop.name).Copy();
